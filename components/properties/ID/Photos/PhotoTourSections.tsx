...
function PhotoTourSectionLayout({ section, baseIndex, openLightbox }: PhotoTourSectionLayoutProps) {
  ...
  // Multi-photo logic for Layout E or Default
const isUneven = section.images.length % 2 !== 0;

if (isUneven) {
  // ODD CASE: 5, 7, 9, 11...
  const [first, second, third, ...others] = section.images;

  return (
    <div className="flex flex-col gap-2 md:gap-4">
      {/* Top Header: Tall image and 2 stacked side images */}
      <div 
        className="grid grid-cols-2 gap-2 md:gap-4"
        style={{ 
          gridTemplateAreas: `"tall top-right" "tall mid-right"`,
      ...
      >
        <button
          ...
          style={{ gridArea: "tall" }}
        >
          <Image src={first} fill className="object-cover" alt="" />
        </button>
        <button
          type="button"
          onClick={() => openLightbox(baseIndex + 1)}
        ...
        >
          <Image src={second} fill className="object-cover" alt="" />
        </button>
        <button
          type="button"
          onClick={() => openLightbox(baseIndex + 2)}
        ...
        >
          <Image src={third} fill className="object-cover" alt="" />
        </button>
      </div>

      {/* Remaining images (4, 6, 8...) displayed in 2x2 style */}
      <div className="grid grid-cols-2 gap-2 md:gap-4">
        {others.map((img, i) => (
          <button
            key={`${section.id}-extra-${i}`}
            type="button"
            onClick={() => openLightbox(baseIndex + 3 + i)}
            className="relative aspect-[4/3] rounded-xl overflow-hidden cursor-pointer block w-full"
          >
...
// EVEN CASE: 4, 6, 8, 10... (Standard Grid)
return (
  <div className="grid grid-cols-2 gap-2 md:gap-4">
