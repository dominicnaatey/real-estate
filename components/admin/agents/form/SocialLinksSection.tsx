...
const inputCls = "w-full h-11 pl-10 pr-4 rounded-(--admin-field-radius) bg-(--admin-field-bg) border border-[#ECECEC] text-sm text-admin-field-text outline-none focus:ring-2 focus:ring-[#008060]/20";
...
function InstagramIcon() {
  return (
    <svg className="w-4 h-4" viewBox="0 0 24 24" fill="url(#ig-gradient)" aria-hidden="true">
      <defs>
    ...
    <div className="space-y-6 bg-white p-4 md:py-10 md:px-6 rounded-(--admin-form-card-radius) border border-[#ECECEC]">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
        <label className="block space-y-2">
          <span className={`block ${labelCls}`}>LinkedIn</span>
        ...
        <label className="block space-y-2">
          <span className={`block ${labelCls}`}>Instagram</span>
          <div className="relative">
            <span className="absolute left-3 top-1/2 -translate-y-1/2 pointer-events-none">
              <InstagramIcon />
            </span>
            <input
              ...
              onChange={(e) => state.setInstagram(e.target.value)}
              className={inputCls}
