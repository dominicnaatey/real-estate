...
const financialsChildren: NavItem[] = [
...
export function Sidebar() {
  ...
  // null = follow pathname, true/false = manually overridden
  const [toggled, setToggled] = useState<boolean | null>(null);
  const financialsOpen = toggled ?? isFinancialsActive;
  ...
  return (
    <aside className="hidden md:flex h-screen w-60 fixed left-0 top-0 bg-white border-r-admin border-admin-border flex-col py-6 z-50">
      <div className="px-6 mb-8 flex flex-col gap-1">
        <h1 className="text-2xl font-serif text-slate-900 tracking-tight">EstatePro</h1>
      ...
      <div className="px-4 mb-6">
        <Link
          ...
          className="w-full bg-[#008060] text-white py-2 px-4 rounded-lg text-sm font-medium flex items-center justify-center gap-2 hover:bg-[#00654b] transition-colors active:scale-95 shadow-sm"
        >
      ...
      <nav className="flex-1 flex flex-col gap-1 px-3">
        ...
        <div>
          ...
          >
            {/* Label — navigates to overview */}
            <button
              type="button"
              onClick={() => { router.push("/admin/financials"); setToggled(true); }}
              className="flex-1 flex items-center gap-3 px-3 py-2.5 transition-transform duration-200 hover:translate-x-1 cursor-pointer"
            >
            ...
            <button
              type="button"
              onClick={() => setToggled((prev) => !(prev ?? isFinancialsActive))}
      ...
      <div className="px-3 pt-4 border-t-admin border-admin-border mt-auto">
        <div className="pt-2 pb-2 px-3">
          <span className="text-[11px] font-semibold text-[#3e4944] uppercase tracking-wider">System</span>
        </div>
